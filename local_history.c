 ...
 */
static nserror
fb_local_history_mouse(struct motif_corewindow *fb_cw,
 ...
 */
static nserror
fb_local_history_key(struct motif_corewindow *fb_cw, uint32_t nskey)
{
	struct fb_local_history_window *lhw;
	/* technically degenerate container of */
	lhw = (struct fb_local_history_window *)fb_cw;
 ...
 */
static nserror
fb_local_history_draw(struct motif_corewindow *fb_cw, struct rect *r)
{
		      ...
		      struct fb_local_history_window **win_out)
{
	...
	res = motif_corewindow_init(parent, &ncwin->core);
	if (res != NSERROR_OK) {
		free(ncwin);
		return res;
	}
...
nserror fb_local_history_destroy(void)
{
	...
	if (res == NSERROR_OK) {
		res = motif_corewindow_fini(&local_history_window->core);
